...
<div class="flex items-center justify-between mb-8">
  <div>
    <h1 style="font-size:1.8rem;font-weight:800;color:#fff;letter-spacing:-0.02em;">
      Messages
    </h1>
...
<div style="background:var(--surface);border:1px solid var(--border);border-radius:8px;overflow:hidden;">
  ...
  <div style="display:grid;grid-template-columns:2fr 2fr 3fr 1fr 1fr;gap:1rem;padding:1.1rem 1.5rem;border-bottom:1px solid var(--border);align-items:center;transition:background 0.2s;{{ !$message->is_read ? 'background:rgba(59,130,246,0.04);' : '' }}"
    ...
    <div style="display:flex;align-items:center;gap:0.6rem;">
      @if(!$message->is_read)
        <div style="width:6px;height:6px;background:var(--blue-bright);border-radius:50%;flex-shrink:0;"></div>
      @else
        <div style="width:6px;height:6px;flex-shrink:0;"></div>
      @endif
      <span style="color:#fff;font-weight:{{ !$message->is_read ? '700' : '400' }};font-size:0.9rem;">
        {{ $message->name }}
      </span>
    </div>

    {{-- Email --}}
    <div style="color:var(--muted);font-size:0.85rem;font-family:'DM Mono',monospace;">
      {{ $message->email }}
    </div>
    ...
    <div>
      ...
      @endif
      <div style="color:var(--muted);font-size:0.8rem;line-height:1.5;">
    ...
    <div style="display:flex;gap:0.5rem;align-items:center;">
      ...
      <button onclick="openMessage({{ $message->id }})"
        ...
        style="width:32px;height:32px;background:rgba(255,255,255,0.05);border:1px solid var(--border);border-radius:6px;color:var(--muted);font-size:0.85rem;cursor:pointer;transition:all 0.2s;display:flex;align-items:center;justify-content:center;"
        onmouseover="this.style.background='rgba(255,255,255,0.1)';this.style.color='#fff'"
        onmouseout="this.style.background='rgba(255,255,255,0.05)';this.style.color='var(--muted)'">
        &#x1F441;
      </button>
  ...
  {{-- Hidden full message data for modal --}}
...
<div id="msgModal"
  ...
  <div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;width:100%;max-width:540px;margin:1.5rem;padding:2rem;position:relative;">
    ...
    <button onclick="closeMessage()"
      style="position:absolute;top:1rem;right:1rem;background:none;border:none;color:var(--muted);font-size:1.3rem;cursor:pointer;line-height:1;">
      &#x2715;
    </button>

    <div style="font-family:'DM Mono',monospace;font-size:0.65rem;letter-spacing:0.18em;text-transform:uppercase;color:var(--blue-bright);margin-bottom:1rem;">
      Full Message
    </div>

    <div style="display:flex;gap:1rem;margin-bottom:1.5rem;padding-bottom:1.5rem;border-bottom:1px solid var(--border);">
      <div style="width:44px;height:44px;background:rgba(59,130,246,0.15);border:1px solid var(--border);border-radius:8px;display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0;">
        &#x1F464;
      </div>
      <div>
